...
interface odernesPageProps{
  orders:Order[]
}

export function OrdenesPage({orders}:odernesPageProps) {
  const [ordersInitial] = useState<Order[]>(mockOrders || orders)
  const [selectedOrder, setSelectedOrder] = useState<Order | null>(null)
  const [filterStatus, setFilterStatus] = useState<string>("all")
  const [searchTerm, setSearchTerm] = useState("")

  const filteredOrders = ordersInitial.filter((order) => {
  ...
  const downloadOrder = (order: Order) => {
    ...
    const orderData = {
      ...
      show: order.play || "N/A",
      date: new Date(order.performance.date).toLocaleDateString("es-ES") || "N/A",
      time: order.performance.time || "N/A",
